...
function send_message($to, $body){
	$CI =& get_instance();
	$CI->load->database();
	$messente_phone_number = $CI->db->query("SELECT * FROM options WHERE option_name = 'messente_phone_number'")->row('option_value');
	$messente_username = $CI->db->query("SELECT * FROM options WHERE option_name = 'messente_username'")->row('option_value');
	$messente_password = $CI->db->query("SELECT * FROM options WHERE option_name = 'messente_password'")->row('option_value');
	...
	$message = array(
		'from'		=> $messente_phone_number,
		'to'		=> $to,
		'content'	=> $body,
	);
	$result = $Messente->send_sms($message);
	return $result;
	if ($result['error_code'] == '' || empty($result['error_code'])){
		return $result['sms_unique_id'];
	} else {
