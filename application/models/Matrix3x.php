<?php
class Matrix3x extends CI_Model {
	...
	public function completed_pairs($username){
		...
		$center_users = count($this->get_leg($username, 'Center'));
		$right_users = count($this->get_leg($username, 'Right'));
		$current_pairs = min($left_users, $center_users, $right_users);
		return $current_pairs;
	}
	public function get_leg($username, $leg){
		...
		if ($binary !== false){
			...
			$childs = $this->getChildren($binary, 'binary_3x');
			if (count($childs)>0){
	...
	public function is_position_available($binary_referral, $position){
		if ($position == 'Left' || $position == 'Right'){
			$query = $this->db->query("SELECT id FROM binary_3x WHERE binary_referral='$binary_referral' AND position='$position'")->result_array();
			if (count($query)>0){
				return false;
			} else {
				return true;
	...
	public function get_binary($username,$position){
		$query = $this->db->query("SELECT username FROM binary_3x WHERE binary_referral='$username' AND position='$position'")->result_array();
		if (count($query)>0){
	...
	public function tree_childs($username, $table = 'binary_3x'){
		$childs = $this->childrens($username, $table = 'binary_3x');
		return count($childs);
	}
	public function direct_childs($username){
		$childs = array();
		$data=$this->db->query("SELECT id, username FROM binary_3x WHERE direct_referral='$username'")->result_array();
		if (count($data)>0){
			foreach($data as $d){
				$childs[] = $d['username'];
			}
	...
	public function binary_childs($username, $table='binary_3x'){
		$data = $this->getChildren($username, $table);
		return $data;
	}
	public function direct_childs_count($username, $table){
		$data=$this->db->query("SELECT id, username FROM binary_3x WHERE direct_referral='$username'")->result_array();
		return count($data);
	}
	public function binary_childs_count($username, $table='binary_3x'){
		$data = $this->getChildren($username, $table);
		return count($data);
	}
	function getOneLevel($username, $table){
		$data=$this->db->query("SELECT id, username FROM binary_3x WHERE binary_referral='$username' ORDER by id ASC")->result_array();
		$data1=array();
		$result = array();
		if (count($data) > 0) {
			foreach ($data as $row){
	...
	function getSingleLevel($username){
		$query=$this->db->query("SELECT username FROM binary_3x WHERE binary_referral='$username' ORDER by id ASC");
		$username=array();
		if ($query->num_rows() > 0) {
	...
	public function find_binary_referral($binery_ref, $position){
		$x = 1;
		do {
			$query = $this->db->query("SELECT * FROM binary_3x WHERE binary_referral='$binery_ref' AND position='$position' ");
			if ($query->num_rows() > 0) {
				foreach ($query->result_array() as $row){
	...
	public function binary_parents($username){
		...
		$x = 1;
		do {
			$data = $this->db->query("SELECT binary_referral FROM binary_3x WHERE username='$username'")->result_array();
			if (count($data)>0){
				if (empty($data[0]['binary_referral'])){
					$x = 0;
				} else {
	...
	public function direct_parents($username){
		...
		$x = 1;
		do {
			$data = $this->db->query("SELECT direct_referral FROM binary_3x WHERE username='$username'")->result_array();
			if (count($data)>0){
	...
	function sponsor($username){
		$query=$this->db->query("SELECT direct_referral FROM binary_3x WHERE username='$username'");
	...
	public function count_direct_ref($username){
		$direct_refs = 0;
		$query = $this->db->query("SELECT count(id) as total FROM binary_3x WHERE direct_referral='$username'");
			if ($query->num_rows() > 0) {
				foreach ($query->result_array() as $row){
